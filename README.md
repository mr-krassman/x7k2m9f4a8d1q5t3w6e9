...
*Статус «значим» — одновременно прошли cross‑asset и out‑of‑time.*
...
*t1⁺/t2⁺ — ⅓ и ⅔ квантили среди dev>0; t1⁻/t2⁻ — ⅓ и ⅔ среди dev<0 (t1⁻ ≤ t2⁻); near = max(Q10(dev), 0.05%). Пороги пересчитываются по истории пары без look-ahead.*
...
*Порог значимости: |Δ| ≥ 1.5 п.п. Колонки скрининга: «Цена росла» / «Цена падала». Индекс = 25%·норм(ср.Δ) + 25%·ср.годы + 25%·ср.пары + 25%·норм(Знач.).*
...
Индекс стабильности EMA

Согласованность знака Δ по календарным годам
...
*Train и val — непересекающиеся когорты пар на одном календарном периоде (2022–2026). Материальность на train: |Δ| ≥ 1.5 п.п. Статус «значим» при p < 0.0036 (Bonferroni, 14 ячеек) на train и ≥ 60% val-пар с тем же знаком Δ, что pooled train.*
...
*Train: 2022-01-01 – 2024-04-01, val: 2024-04-01 – 2026-05-31 — одни и те же 49 пар. Подтверждение: ≥ 60% пар сохраняют знак Δ во втором периоде.*
...
Δ train vs val по сигналам EMA(5)

*Рис. EMA-1. Отклонение доли «Цена росла» / «Цена падала» от BASE на train и val (п.п.). Зелёная заливка — итоговый статус «значим».*
...
*Сводка за полный период 2022–2026: pooled Δ к BASE; «Годы» и «Пары» — repeatability знака Δ (порог 40/15 дн. в году); n бакета — pooled число дней-наблюдений по всем парам. Статус «значим» — пересечение проверок 1 и 2.*
