"""Адаптер стратегии ema_spreads для backtester."""
...
from crypto_research.utils.backtest.strategies.ema_spreads import (
    EmaSpreadsBacktestContext,
    STRATEGY_NAME,
    WARMUP_CALENDAR_DAYS,
    compute_frozen_thresholds,
    run_ema_spreads_backtest,
)
...
class EmaSpreadsStrategyHandler(StrategyHandler):
    name = STRATEGY_NAME

    def validate_args(self, parser: argparse.ArgumentParser, args: argparse.Namespace) -> None:
        period = getattr(args, "ema_period", SELECTED_EMA_PERIOD)
        if period != SELECTED_EMA_PERIOD:
            parser.error(
                f"Бэктест ema_spreads поддерживает только EMA({SELECTED_EMA_PERIOD}), "
                f"получено: {period}"
            )
    ...
    def prepare(self, ctx: BacktestContext) -> StrategyPrepareResult:
        ...
        frozen_thresholds = compute_frozen_thresholds(train_daily, period)
        ...
        if ctx.scenario == SCENARIO_OPTIMISTIC:
            selected = resolve_optimistic_ema_pairs(
                ctx.data_dir,
                period=period,
                workers=ctx.workers,
            )
    ...
    def run(
        ...
        ema_ctx = EmaSpreadsBacktestContext(
        ...
        return run_ema_spreads_backtest(daily, pairs, ema_ctx)
