"""Стратегия volume_spreads: long intraday при вчерашнем b2 HIGH (momentum)."""
...
from crypto_research.utils.volume.pair_selection import VOLUME_BUCKET_B2, VOLUME_TRAIN_SIGNAL
from crypto_research.utils.volume.volume import build_volume_work_frame, vol_log_rel_prev_column

log = get_logger("strategy_volume_spreads")

STRATEGY_NAME = "volume_spreads"
TRADING_SEGMENTS: tuple[str, ...] = ("b2",)
WARMUP_CALENDAR_DAYS = 70

STRATEGY_DESCRIPTION_MAXIMAL = (
    "Intraday UTC, равный вес пар, без реинвестирования. Сигнал — исследование volume_spreads.\n"
    f"  EMA(volume, {SELECTED_VOLUME_EMA_PERIOD}): если вчера ln(vol/EMA) попал в b2 HIGH → long open→close.\n"
    "  Пороги q30/q70/q85/q95 per-pair заморожены на train (2022-01-01 – 2024-04-01).\n"
    "  Остальные дни: flat."
)

STRATEGY_DESCRIPTION_CONSERVATIVE = (
    f"Консервативный сценарий: val-период, все 49 пар, EMA(volume, {SELECTED_VOLUME_EMA_PERIOD}), "
    "long при вчерашнем b2 HIGH. Пороги frozen train.\n"
    "  Доходность long = (close−open)/open×100%.\n"
    "  Остальные дни: flat."
)

STRATEGY_DESCRIPTION_OPTIMISTIC = (
    f"Оптимистичный сценарий: val, train-отбор пар по b2 HIGH × «Цена росла» "
    f"(знак Δ + ≥2/3 лет). EMA(volume, {SELECTED_VOLUME_EMA_PERIOD}), пороги frozen train.\n"
    "  Long open→close при вчерашнем b2 HIGH.\n"
    "  Остальные дни: flat."
)


@dataclass(frozen=True)
class VolumeSpreadsBacktestContext:
    data_dir: object
    from_date: datetime
    to_date: datetime
    pairs: list[str]
...
def compute_frozen_volume_thresholds(daily_train: pl.DataFrame, period: int) -> pl.DataFrame:
    work = build_volume_work_frame(daily_train, period)
    prev_col = vol_log_rel_prev_column(period)
    return build_pair_volume_thresholds_frame(work, prev_col)
...
def build_pair_returns(
    ...
    *,
    ...
    log_prev = merged[prev_col].to_numpy().astype(np.float64, copy=False)
    buckets = assign_volume_buckets_vectorized(
        log_prev,
        merged["q30"].to_numpy().astype(np.float64, copy=False),
        merged["q70"].to_numpy().astype(np.float64, copy=False),
        merged["q85"].to_numpy().astype(np.float64, copy=False),
        merged["q95"].to_numpy().astype(np.float64, copy=False),
    )
    position = np.where(buckets == VOLUME_BUCKET_B2, 1.0, 0.0)
    df = merged.with_columns(
        pl.Series("vol_bucket_prev", buckets),
...
def _strategy_description(ctx: VolumeSpreadsBacktestContext) -> str:
...
def run_volume_spreads_backtest(
    daily: pl.DataFrame,
    pairs: list[str],
    ctx: VolumeSpreadsBacktestContext,
) -> BacktestResult:
    if ctx.frozen_thresholds is None or ctx.frozen_thresholds.is_empty():
        raise RuntimeError("frozen_thresholds не заданы для volume_spreads backtest")
    ...
    result = BacktestResult(
        ...
        exposure_note=(
            f"Капитал в рынке только в дни с сигналом {VOLUME_TRAIN_SIGNAL.label} "
            f"(вчера HIGH volume, EMA(volume, {ctx.vol_period}))."
        ),
