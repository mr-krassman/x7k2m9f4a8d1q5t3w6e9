"""Адаптер стратегии rsi_spreads для backtester."""
...
from crypto_research.utils.backtest.strategies.rsi_spreads import (
    RsiSpreadsBacktestContext,
    STRATEGY_NAME,
    WARMUP_CALENDAR_DAYS,
    compute_frozen_rsi_edges,
    run_rsi_spreads_backtest,
)
...
class RsiSpreadsStrategyHandler(StrategyHandler):
    name = STRATEGY_NAME

    def validate_args(self, parser: argparse.ArgumentParser, args: argparse.Namespace) -> None:
        period = getattr(args, "rsi_period", SELECTED_RSI_PERIOD)
        if period != SELECTED_RSI_PERIOD:
            parser.error(
                f"Бэктест rsi_spreads поддерживает только RSI({SELECTED_RSI_PERIOD}), "
                f"получено: {period}"
            )
    ...
    def prepare(self, ctx: BacktestContext) -> StrategyPrepareResult:
        ...
        frozen_edges = compute_frozen_rsi_edges(train_daily, period)
        ...
        if ctx.scenario == SCENARIO_OPTIMISTIC:
            selected = resolve_optimistic_rsi_pairs(
                ctx.data_dir,
                period=period,
                workers=ctx.workers,
            )
    ...
    def run(
        ...
        rsi_ctx = RsiSpreadsBacktestContext(
        ...
        return run_rsi_spreads_backtest(daily, pairs, rsi_ctx)
