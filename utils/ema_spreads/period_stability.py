"""Метрики стабильности периода EMA — та же repeatability, что в ema_spreads."""
...
def _material_cells_for_period(
    ...
    for bucket in range(N_BUCKETS):
        for col in SCREEN_STATS_COLS:
            ...
            p_eligible = _pair_eligible_count(
...
def material_cells_for_period(
    ...
    return _material_cells_for_period(daily, period, periods, pair_bands)
...
def yearly_material_agreement_rates(
    ...
    """Доля материальных ячеек, где знак Δ в году совпал с полным периодом."""
    cells = _material_cells_for_period(daily, period, periods, pair_bands)
    if not cells:
        return []
    prepared = prepare_ema_condition_frame(daily, period, pair_bands, periods)
