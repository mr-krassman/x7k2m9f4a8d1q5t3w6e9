"""Сводный отчёт volume_spreads: универсальность среди пар и устойчивость во времени."""
...
from crypto_research.utils.pipeline.paths import volume_summary_log_path, volume_summary_plot_path
...
from crypto_research.utils.volume.summary import (
    VOLUME_SIGNAL_SPEC,
    compute_volume_signal_pooled_summary,
    compute_volume_signal_summary,
    format_volume_pooled_signal_table,
    format_volume_signal_summary_table,
    status_by_signal_key,
)

log = get_logger("volume_train_val_summary")
...
def _compute_summary_rows(
    ...
    *,
    ...
    pair_rows = compute_volume_signal_summary(
        train_daily,
        val_daily,
        train_bands,
        val_bands,
    ...
    temporal_rows = compute_volume_signal_summary(
...
def run_summary_report(
    ...
    vol_period = ctx.ema_periods[0] if ctx.ema_periods else SELECTED_VOLUME_EMA_PERIOD
    ...
    parts = [
        f"=== Отчёт: сводка EMA(volume,{vol_period}) — подтверждение сигналов ===",
        "",
    ...
    pooled_rows = compute_volume_signal_pooled_summary(
        full_daily,
        full_bands,
        status_by_key=status_map,
        period=vol_period,
    )
    parts.extend(format_volume_pooled_signal_table(pooled_rows, vol_period=vol_period))

    plot_check1 = volume_summary_plot_path(vol_period, "check_pair_universality_delta")
    plot_check2 = volume_summary_plot_path(vol_period, "check_temporal_stability_delta")
    save_check_signal_delta_chart(
        pair_rows,
        plot_check1,
        title=f"EMA(volume,{vol_period}) — проверка 1: универсальность среди пар",
        subtitle="зелёная полоса = статус «значим» (24 train / 25 val, 2022–2026)",
    )
    save_check_signal_delta_chart(
        temporal_rows,
        plot_check2,
        title=f"EMA(volume,{vol_period}) — проверка 2: устойчивость во времени",
        subtitle="зелёная полоса = статус «значим» (49 пар, train 2022–2024-04 / val 2024-04–2026)",
    )

    path = volume_summary_log_path(vol_period)
    lines = parts + [
        "",
        "=== Графики ===",
    ...
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    log.info("Сводка EMA(volume) сохранена: %s", path)
    return path
