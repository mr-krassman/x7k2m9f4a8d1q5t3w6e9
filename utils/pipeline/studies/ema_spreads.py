...
from crypto_research.utils.ema_spreads.report import run_ema_spreads_report
from crypto_research.utils.pipeline.dates import parse_iso_utc
from crypto_research.utils.pipeline.ema_train_val_summary import run_summary_report
...
class EmaSpreadsStudy(StudyHandler):
    ...
    def validate_args(self, parser: argparse.ArgumentParser, args: argparse.Namespace) -> None:
        if not args.summary and (not args.from_date or not args.to_date):
            parser.error("ema_spreads: --from-date и --to-date обязательны вне режима --summary")
