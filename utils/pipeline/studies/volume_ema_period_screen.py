"""Этап 0: выбор периода EMA объёма по стабильности сигнала."""
...
from crypto_research.utils.volume.period_screen_report import run_volume_ema_period_screen_report


class VolumeEmaPeriodScreenStudy(StudyHandler):
    supports_summary = False

    def validate_args(self, parser: argparse.ArgumentParser, args: argparse.Namespace) -> None:
        if args.summary:
            parser.error("volume_ema_period_screen не поддерживает --summary")
        if args.train or args.val:
            parser.error("volume_ema_period_screen не поддерживает --train / --val")
    ...
    def run(self, ctx: ReportContext, dataset: StudyDataset) -> Path:
        return run_volume_ema_period_screen_report(
            dataset.daily,
            dataset.pairs,
            dataset.pair_bands,
            ctx.from_date,
    ...
    def run_summary(self, ctx: ReportContext, summary: SummaryDatasets) -> Path:
        raise NotImplementedError("volume_ema_period_screen не поддерживает --summary")
