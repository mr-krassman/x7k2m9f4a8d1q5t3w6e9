"""Исследование volume_spreads — подтверждение сигналов ln(volume/EMA)."""
...
from crypto_research.utils.pipeline.volume_train_val_summary import run_summary_report


class VolumeSpreadsStudy(StudyHandler):
    supports_summary = True

    def validate_args(self, parser: argparse.ArgumentParser, args: argparse.Namespace) -> None:
        if not args.summary:
            parser.error("volume_spreads: используйте --summary (детальный отчёт — позже)")
        if args.train or args.val:
            parser.error("volume_spreads не поддерживает --train / --val")
    ...
    def run(self, ctx: ReportContext, dataset: StudyDataset) -> Path:
        raise NotImplementedError("volume_spreads: используйте --summary")
