"""Исследование candlestick harami (Bullish / Bearish)."""
...
from crypto_research.utils.pipeline.harami_train_val_summary import run_summary_report
...
class HaramiStudy(StudyHandler):
    supports_summary = True

    def validate_args(self, parser: argparse.ArgumentParser, args: argparse.Namespace) -> None:
        if not args.summary:
            parser.error("harami: используйте --summary (детальный отчёт по периоду — позже)")
    ...
    def run(self, ctx: ReportContext, dataset: StudyDataset) -> Path:
        del ctx, dataset
        raise RuntimeError("harami: только режим --summary")
