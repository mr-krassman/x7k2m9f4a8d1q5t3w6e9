"""Сводный отчёт ema_harami: EMA(9) бакеты + фильтр Harami."""
...
from crypto_research.utils.ema_harami.constants import SELECTED_EMA_PERIOD, STUDY_NOTE
from crypto_research.utils.ema_harami.sample_candles_plot import save_ema_harami_sample_candles_plot
from crypto_research.utils.ema_harami.summary import (
    compute_ema_harami_signal_pooled_summary,
    compute_ema_harami_signal_summary,
    format_ema_harami_pooled_signal_table,
    format_ema_harami_signal_summary_table,
...
from crypto_research.utils.pipeline.paths import (
    ema_harami_sample_candles_plot_path,
    ema_harami_summary_log_path,
    ema_harami_summary_plot_path,
)
...
log = get_logger("ema_harami_train_val_summary")
...
def _compute_summary_rows(
    ...
    pair_rows = compute_ema_harami_signal_summary(
        train_daily,
        val_daily,
        train_bands,
        val_bands,
    ...
    temporal_rows = compute_ema_harami_signal_summary(
...
def assemble_summary_report(
    ...
    parts = [
        f"=== Отчёт: сводка EMA({ema_period}) + Harami — подтверждение сигналов ===",
        "",
        STUDY_NOTE,
        "",
        "Три блока (как ema_spreads / harami):",
        ...
        "",
        f"Для сравнения с чистой EMA: research_outputs/ema/ema_spreads/statistics/"
        f"ema_summary_ema{ema_period}.log",
        "",
    ]
    ...
    pooled_rows = compute_ema_harami_signal_pooled_summary(
        full_daily,
        full_bands,
        status_by_key=status_map,
    )
    parts.extend(format_ema_harami_pooled_signal_table(pooled_rows, ema_period=ema_period))
    return "\n".join(parts)
...
def save_summary_report(text: str, path: Path) -> Path:
    ...
    log.info("[ema_harami] сводка сохранена: %s", path)
    return path
...
def run_summary_report(
    ...
    ema_period = SELECTED_EMA_PERIOD
    plot_check1 = ema_harami_summary_plot_path(ema_period, "check_pair_universality_delta")
    plot_check2 = ema_harami_summary_plot_path(ema_period, "check_temporal_stability_delta")
    save_check_signal_delta_chart(
        pair_rows,
        plot_check1,
        title=f"ema_harami EMA({ema_period}) — проверка 1: универсальность среди пар",
        subtitle="зелёная полоса = статус «значим» (train/val когорты, 2022–2026)",
    )
    save_check_signal_delta_chart(
        temporal_rows,
        plot_check2,
        title=f"ema_harami EMA({ema_period}) — проверка 2: устойчивость во времени",
        subtitle="зелёная полоса = статус «значим» (49 пар, train 2022–2024-04 / val 2024-04–2026)",
    )

    plot_candles = ema_harami_sample_candles_plot_path(ema_period)
    save_ema_harami_sample_candles_plot(full_daily, plot_candles, period=ema_period)
    ...
    lines.extend([
        ...
        f"Свечи ±10 дней вокруг t+1 (20 Bullish + 20 Bearish): {plot_candles}",
        "",
    ])
    return save_summary_report("\n".join(lines), ema_harami_summary_log_path(ema_period))
