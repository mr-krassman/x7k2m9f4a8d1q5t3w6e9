"""Сводный отчёт price_sequences: универсальность среди пар и устойчивость во времени."""
...
from crypto_research.utils.pipeline.paths import (
    price_sequences_summary_log_path,
    price_sequences_summary_plot_path,
)
from crypto_research.utils.pipeline.report_context import ReportContext
from crypto_research.utils.price_sequences.summary import (
    compute_price_sequence_signal_pooled_summary,
    compute_price_sequence_signal_summary,
    format_price_sequence_pooled_signal_table,
    format_price_sequence_signal_summary_table,
...
log = get_logger("price_sequences_train_val_summary")
...
def _compute_summary_rows(
    ...
    pair_rows = compute_price_sequence_signal_summary(
        train_daily,
        val_daily,
        train_bands,
        val_bands,
    ...
    temporal_rows = compute_price_sequence_signal_summary(
...
def assemble_summary_report(
    ...
    parts = [
        "=== Отчёт: сводка price_sequences — подтверждение сигналов ===",
        "",
    ...
    pooled_rows = compute_price_sequence_signal_pooled_summary(
        full_daily,
        full_bands,
        status_by_key=status_map,
    )
    parts.extend(format_price_sequence_pooled_signal_table(pooled_rows))
    return "\n".join(parts)
...
def save_summary_report(text: str, path: Path) -> Path:
    ...
    log.info("[price_sequences] сводка сохранена: %s", path)
    return path
...
def run_summary_report(
    ...
    plot_check1 = price_sequences_summary_plot_path("check_pair_universality_delta")
    plot_check2 = price_sequences_summary_plot_path("check_temporal_stability_delta")
    save_check_signal_delta_chart(
        pair_rows,
        plot_check1,
        title="price_sequences — проверка 1: универсальность среди пар",
        subtitle="зелёная полоса = статус «значим» (train/val когорты, 2022–2026)",
    )
    save_check_signal_delta_chart(
        temporal_rows,
        plot_check2,
        title="price_sequences — проверка 2: устойчивость во времени",
    ...
    return save_summary_report("\n".join(lines), price_sequences_summary_log_path())
