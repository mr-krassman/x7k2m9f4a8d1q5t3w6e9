"""Графики сводки подтверждения сигналов spread-исследований."""
...
from crypto_research.utils.research.spread_summary import SignalSummaryRow

log = get_logger("spread_summary_plots")
...
def save_check_signal_delta_chart(
    ...
    *,
    ...
    if not rows:
        return
    labels = [f"b{r.key.bucket}·{r.key.column[:6]}" for r in rows]
    train_vals = [r.train_delta_pp for r in rows]
    val_vals = [
        r.val_delta_pp if r.val_delta_pp == r.val_delta_pp else 0.0 for r in rows
    ]
