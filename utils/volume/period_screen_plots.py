"""Графики стабильности для этапа выбора периода EMA объёма."""
...
log = get_logger("volume_ema_period_screen_plots")
...
def save_stability_index_chart(
    ...
    *,
    ...
    labels = [f"vol EMA({m.period})" for m in ordered]
    ...
    ax.set_title(
        "Выбор периода EMA(volume) — индекс: ср.|Δ|, кварталы, пары, значимые ячейки (по 25%)\n"
    ...
    for i, (val, m) in enumerate(zip(values, ordered, strict=True)):
        ax.text(val + 1.0, i, f"{val:.1f}%  (#{m.rank})", va="center", fontsize=9)
    fig.tight_layout()
    fig.savefig(path, dpi=PLOT_DPI, bbox_inches="tight")
    plt.close(fig)
    log.info("График индекса стабильности EMA(volume): %s", path)
...
def save_yearly_agreement_heatmap(
    ...
    *,
    ...
    for period in periods:
        rows = yearly_material_agreement_rates(daily, period, pair_bands)
    ...
    ax.set_yticks(np.arange(len(period_list)), [f"vol EMA({p})" for p in period_list])
    ax.set_xlabel("Год (UTC)")
    ax.set_ylabel("Период EMA(volume)")
    ax.set_title(
    ...
    fig.savefig(path, dpi=PLOT_DPI, bbox_inches="tight")
    plt.close(fig)
    log.info("Heatmap согласия по годам EMA(volume): %s", path)
