"""Отчёт этапа 0: выбор периода EMA объёма по стабильности сигнала."""
...
from crypto_research.utils.pipeline.paths import (
    volume_ema_period_screen_log_path,
    volume_ema_period_screen_plot_path,
)
...
from crypto_research.utils.volume.period_screen_plots import (
    save_stability_index_chart,
    save_yearly_agreement_heatmap,
)
from crypto_research.utils.volume.period_stability import (
...
log = get_logger("volume_ema_period_screen")
_PAIRS_PER_LINE = 11

_METHOD = (
    f"Этап 0 — выбор периода EMA(volume). Колонки: «{SCREEN_STATS_COLS[0]}» и "
...
def _summary_table_lines(ranked: list) -> list[str]:
    ...
    for m in ranked:
        lines.append(
            f"{m.rank:>{col_rank}} | {m.period:>{col_vol}} | "
    ...
    lines.extend(
        [
            ...
            "",
            (
                f"Рекомендация этапа 0: EMA(volume,{ranked[0].period}) — индекс "
...
def _material_detail_lines(
    ...
    for m in ranked:
        ...
        lines.append(f"--- EMA(volume,{m.period}) ---")
...
def run_volume_ema_period_screen_report(
    daily: pl.DataFrame,
    pairs: list[str],
    pair_bands: dict[str, MeanBands],
    from_date: datetime,
    ...
    screen_periods = periods or DEFAULT_SCREEN_VOLUME_EMA_PERIODS
    raw_metrics = []
    ...
    if not raw_metrics:
        raise RuntimeError("Недостаточно данных для скрининга периодов EMA(volume)")

    ranked = rank_period_stabilities(raw_metrics)

    header = [
        "=== Отчёт: volume_ema_period_screen (этап 0 — выбор периода EMA объёма) ===",
        f"Пар: {len(pairs)}",
        f"Период скрининга (UTC, train): {from_date:%Y-%m-%d} .. {to_date:%Y-%m-%d}",
        f"Кандидаты EMA(volume): {', '.join(str(p) for p in screen_periods)}",
    ]
    ...
    plot_index = volume_ema_period_screen_plot_path(
        len(pairs), from_date, to_date, screen_periods, "stability_index"
    )
    plot_heat = volume_ema_period_screen_plot_path(
    ...
    path = volume_ema_period_screen_log_path(len(pairs), from_date, to_date, screen_periods)
    path.parent.mkdir(parents=True, exist_ok=True)
    text = "\n".join(header + body)
    path.write_text(text + "\n", encoding="utf-8")
    log.info("Отчёт выбора периода EMA(volume): %s", path)
    return path
