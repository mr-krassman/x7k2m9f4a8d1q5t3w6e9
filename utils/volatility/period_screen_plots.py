"""Графики стабильности для этапа выбора периода SMA(range)."""
...
log = get_logger("volatility_period_screen_plots")
...
def save_stability_index_chart(
    ...
    *,
    ...
    labels = [f"SMA({m.period})" for m in ordered]
    ...
    ax.set_title(
        "Выбор периода SMA(range) — индекс: ср.|Δ|, кварталы, пары, значимые ячейки (по 25%)\n"
    ...
    plt.close(fig)
    log.info("График индекса стабильности SMA(range): %s", path)
...
def save_yearly_agreement_heatmap(
    ...
    *,
    ...
    ax.set_yticks(np.arange(len(period_list)), [f"SMA({p})" for p in period_list])
    ax.set_xlabel("Год (UTC)")
    ax.set_ylabel("Период SMA(range)")
    ax.set_title(
    ...
    fig.savefig(path, dpi=PLOT_DPI, bbox_inches="tight")
    plt.close(fig)
    log.info("Heatmap согласия по годам SMA(range): %s", path)
