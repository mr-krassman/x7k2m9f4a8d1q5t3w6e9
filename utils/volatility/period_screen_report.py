"""Отчёт этапа 0: выбор периода SMA(range) по стабильности сигнала."""
...
from crypto_research.utils.pipeline.paths import (
    volatility_period_screen_log_path,
    volatility_period_screen_plot_path,
)
...
from crypto_research.utils.volatility.period_screen_plots import (
    save_stability_index_chart,
    save_yearly_agreement_heatmap,
)
from crypto_research.utils.volatility.period_stability import (
...
log = get_logger("volatility_period_screen")
_PAIRS_PER_LINE = 11

_METHOD = (
    f"Этап 0 — выбор периода SMA(range). Колонки: «{SCREEN_STATS_COLS[0]}» и "
...
def _summary_table_lines(ranked: list) -> list[str]:
    ...
    for m in ranked:
        lines.append(
            f"{m.rank:>{col_rank}} | {m.period:>{col_sma}} | "
    ...
    lines.extend(
        [
            ...
            "",
            (
                f"Рекомендация этапа 0: SMA({ranked[0].period}) — индекс "
            ...
            "Дальше: volatility_spreads с выбранным периодом и проверки train/val.",
            "",
        ]
    )
    return lines
...
def _material_detail_lines(
    ...
    for m in ranked:
        ...
        lines.append(f"--- SMA(range,{m.period}) ---")
...
def run_volatility_period_screen_report(
    daily: pl.DataFrame,
    pairs: list[str],
    pair_bands: dict[str, MeanBands],
    from_date: datetime,
    ...
    screen_periods = periods or DEFAULT_SCREEN_RANGE_SMA_PERIODS
    raw_metrics = []
    ...
    if not raw_metrics:
        raise RuntimeError("Недостаточно данных для скрининга периодов SMA(range)")

    ranked = rank_period_stabilities(raw_metrics)

    header = [
        "=== Отчёт: volatility_period_screen (этап 0 — выбор периода SMA(range)) ===",
        f"Пар: {len(pairs)}",
        f"Период скрининга (UTC, train): {from_date:%Y-%m-%d} .. {to_date:%Y-%m-%d}",
        f"Кандидаты SMA(range): {', '.join(str(p) for p in screen_periods)}",
    ]
    ...
    plot_index = volatility_period_screen_plot_path(
        len(pairs), from_date, to_date, screen_periods, "stability_index"
    )
    plot_heat = volatility_period_screen_plot_path(
    ...
    path = volatility_period_screen_log_path(len(pairs), from_date, to_date, screen_periods)
    path.parent.mkdir(parents=True, exist_ok=True)
    text = "\n".join(header + body)
    path.write_text(text + "\n", encoding="utf-8")
    log.info("Отчёт выбора периода SMA(range): %s", path)
    return path
