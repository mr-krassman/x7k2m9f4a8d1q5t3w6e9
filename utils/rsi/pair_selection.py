"""Отбор пар по train для optimistic-сценария rsi_spreads (b0 × Цена росла)."""
...
RSI_BUCKET_B0 = 0
RSI_SIGNAL_COLUMN = RETURN_STATS_COLS[0]


@dataclass(frozen=True)
class RsiTrainSignal:
    bucket: int
    column: str
    direction: str
    label: str
...
RSI_TRAIN_SIGNAL = RsiTrainSignal(
    bucket=RSI_BUCKET_B0,
    column=RSI_SIGNAL_COLUMN,
    direction="long",
    label="b0",
)

RSI_TRAIN_SIGNALS: tuple[RsiTrainSignal, ...] = (RSI_TRAIN_SIGNAL,)


@dataclass(frozen=True)
class RsiPairSelection:
    signal: RsiTrainSignal
...
def select_rsi_pairs_for_signal(
    daily: pl.DataFrame,
    pair_bands: dict[str, MeanBands],
    signal: RsiTrainSignal,
    period: int = SELECTED_RSI_PERIOD,
    *,
    min_year_num: int = 2,
    min_year_den: int = 3,
) -> RsiPairSelection:
...
def select_rsi_train_pairs(
    ...
    *,
    min_year_num: int = 2,
    min_year_den: int = 3,
) -> list[RsiPairSelection]:
    return [
        select_rsi_pairs_for_signal(
            daily,
            pair_bands,
            signal,
            period,
        ...
        for signal in RSI_TRAIN_SIGNALS
    ]


def resolve_optimistic_rsi_pairs(
    data_dir,
    *,
    period: int = SELECTED_RSI_PERIOD,
    workers: int,
    ...
    selections = select_rsi_train_pairs(
        daily,
        pair_bands,
        period,
        min_year_num=min_year_num,
