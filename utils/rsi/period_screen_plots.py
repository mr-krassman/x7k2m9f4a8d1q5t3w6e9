"""Графики стабильности для этапа выбора периода RSI."""
...
log = get_logger("rsi_period_screen_plots")
...
def save_stability_index_chart(
    ...
    *,
    ...
    labels = [f"RSI({m.period})" for m in ordered]
    ...
    ax.set_title(
        "Выбор периода RSI — индекс: ср.|Δ|, кварталы, пары, значимые ячейки (по 25%)\n"
    ...
    plt.close(fig)
    log.info("График индекса стабильности RSI: %s", path)
...
def save_yearly_agreement_heatmap(
    ...
    *,
    ...
    ax.set_yticks(np.arange(len(period_list)), [f"RSI({p})" for p in period_list])
    ax.set_xlabel("Год (UTC)")
    ax.set_ylabel("Период RSI")
    ax.set_title(
    ...
    fig.savefig(path, dpi=PLOT_DPI, bbox_inches="tight")
    plt.close(fig)
    log.info("Heatmap согласия по годам RSI: %s", path)
